/********************************************************************************
 * Л А Б О Р А Т О Р Н А Я   Р А Б О Т А  №7  З А   5   С Е М Е С Т Р   С  +  + *
 *------------------------------------------------------------------------------*
 * Project Type  : Win64 Console Application                                    *
 * Project Name  : Teor_Grph_Lab7                                               *
 * File Name     : Lab7. CPP                                                    *
 * Programmer(s) : Викулов Д.Г.                                                 *
 * Modifyed By   :                                                              *
 * Created       : 19/09/23                                                     *
 * Last Revision : 12/10/23                                                     *
 * Comment(s)    : Программа, рассчитывающая расстояние между всеми парами      *
 *                 вершин в графе. Поиск производится алгоритмом Джонсона.      *
 *                 Результатом работы является перечисление пар вершин, и       *
 *                 соответствующих расстояний между ними.                       *
 ********************************************************************************/
...
const char* FNAMEIN = "matrix_t7_001.txt";                // файл с матрицей смежности
//const char* FNAMEIN = "matrix_t7_002.txt";                // файл с матрицей смежности
//const char* FNAMEIN = "matrix_t7_003.txt";                // файл с матрицей смежности
//const char* FNAMEIN = "matrix_t7_004.txt";                // файл с матрицей смежности
//const char* FNAMEIN = "matrix_t7_005.txt";                // файл с матрицей смежности
//const char* FNAMEIN = "matrix_t7_006.txt";                // файл с матрицей смежности
//const char* FNAMEIN = "matrix_t7_007.txt";                // файл с матрицей смежности
//const char* FNAMEIN = "matrix_t7_008.txt";                // файл с матрицей смежности
//const char* FNAMEIN = "matrix_t7_009.txt";                // файл с матрицей смежности
//const char* FNAMEIN = "matrix_t7_010.txt";                // файл с матрицей смежности

// ТЕСТЫ ДЛЯ СПИСКА РЕБЕР
//const char* FNAMEIN = "list_of_edges_t7_001.txt";         // файл со списком рёбер
//const char* FNAMEIN = "list_of_edges_t7_002.txt";         // файл со списком рёбер
//const char* FNAMEIN = "list_of_edges_t7_003.txt";         // файл со списком рёбер
//const char* FNAMEIN = "list_of_edges_t7_004.txt";         // файл со списком рёбер
//const char* FNAMEIN = "list_of_edges_t7_005.txt";         // файл со списком рёбер
//const char* FNAMEIN = "list_of_edges_t7_006.txt";         // файл со списком рёбер
//const char* FNAMEIN = "list_of_edges_t7_007.txt";         // файл со списком рёбер
//const char* FNAMEIN = "list_of_edges_t7_008.txt";         // файл со списком рёбер
//const char* FNAMEIN = "list_of_edges_t7_009.txt";         // файл со списком рёбер
//const char* FNAMEIN = "list_of_edges_t7_010.txt";         // файл со списком рёбер
...
// Вспомогательная функция (используется в ДЕЙКСТРЕ) для нахождения индекса вершины с минимальным расстоянием
...
void Dijkstra(vector<vector<int>> matrix,                         // исходная матрица для определения наличия пути между вершинами
	          vector<vector<int>> graph,                          // скорректированная матрица смежности (отсутствуют рёбра отрицательного веса)
...
void BellForMur(vector<vector<int>>graph,                         // матрица смежности графа с добавленной нулевой вершиной
	            vector<int>& distances,                           // кратчайшие расстояния от этой вершины до всех остальных
	            int start);                                       // индекс добавленной вершины

// Алгоритм Джонсона  
void Djonson(vector<vector<int>> graph,                           // матрица смежности графа, к которому применяем алгоритм
...
{
	cout << "\n\t\t\t\t/********************************************************************************\n";
	cout << "\t\t\t\t* Л А Б О Р А Т О Р Н А Я   Р А Б О Т А  №7  З А   5   С Е М Е С Т Р   С  +  +  *\n";
	cout << "\t\t\t\t*-------------------------------------------------------------------------------*\n";
	cout << "\t\t\t\t* Project Type  : Win64 Console Application                                     *\n";
	cout << "\t\t\t\t* Project Name  : Teor_Grph_Lab7                                                *\n";
	cout << "\t\t\t\t* File Name     : Lab7. CPP                                                     *\n";
	cout << "\t\t\t\t* Programmer(s) : Викулов Д.Г.  &                                               *\n";
	cout << "\t\t\t\t* Modifyed By   :                                                               *\n";
	cout << "\t\t\t\t* Created       : 19/09/23                                                      *\n";
	cout << "\t\t\t\t* Last Revision : 11/10/23                                                      *\n";
	cout << "\t\t\t\t********************************************************************************/\n";
	...
	vector<int>& ways)                                  // расстояние от данной вершины до всех остальных в графе
{
	...
	for (int i = 0; i < Vertices - 1; i++)
	{
		...
		{	// перебираем уже все вершины смежные с данной(с наименьшим весом) по матрице смежности по следующим правилам:
			// условие существования пути между вершинами смотрится по матрице исходного графа, т.к. некоторые пути могут обнулится после корректировки, но нужно их учитывать тоже
			...
			if (matrix[current][j] != 0 && visited[j] == false && distance[current] + graph[current][j] < distance[j])
			{
	...
	// запоминаем путь от данной вершины дов сех остальных
	ways = distance;
...
void BellForMur(vector<vector<int>>graph,                         // матрица смежности графа с добавленной нулевой вершиной
	            ...
	            int start)                                        // индекс добавленной вершины
{
	int V = distances.size();                                     // количество вершин в графе с одной добавленной
	distances[start] = 0;                                         // пути из стартовой вершины в саму себя нету -> путь равен нулю

	// Начинаем алгоритм всегда со стартовой вершины, так как расстояние от неё до всех остальных - нулевое(по условию алгоритма), то сразу заполняем нулями вектор дистанций
	for (int i = 0; i < V; i++)
	{
		distances[i] = 0;
	}// for i
...
void Djonson(vector<vector<int>> graph,                           // матрица смежности графа, к которому применяем алгоритм
	         bool print)                                          // флаг печати в файл
{
	int V = graph.size();                                         // определяем текущий размер матрицы смежности графа
	// массив расстояний. Вводим на одну вершину больше, так как её будеи добавлять
	vector<int> distances(V + 1, INF);
	vector<vector<int>> matrix(V + 1, vector<int>(V + 1, 0));         // матрица смежности из нулей с добавленной вершиной из нулей
	vector<vector<int>> predok = graph;                           // матрица смежности с добавленной вершиной и редактированными неотрицательными весами

	// Добавляем в вектор вершину с нулевыми расстояниями
	...
	// Запускаем алгоритм Беллмана-Форда-Мура. Находим кратчайшие расстояния от новой вершины до всех остальных
	BellForMur(matrix, distances, V);

	cout << "\n\x1b[36m К Р А Т Ч А Й Ш И Е   Р А С С Т О Я Н И Я   О Т   Д О Б А В Л Е Н Н О Й   В Е Р Ш И Н Ы   Д О   В С Е Х   О С Т А Л Ь Н Ы Х:\x1b[30m\n";
	cout << " ";
	// вычисленные величины h
	...
	// выводим то же самое в файл
	if (print)
	{
		fout << "\nК Р А Т Ч А Й Ш И Е   Р А С С Т О Я Н И Я   О Т   Д О Б А В Л Е Н Н О Й   В Е Р Ш И Н Ы   Д О   В С Е Х   О С Т А Л Ь Н Ы Х:\n";
		fout << " ";
		// вычисленные величины h
		for (int i = 0; i < distances.size(); i++)
		{
	...
	// редактирование весов рёбер на h - получение неотрицательных весов рёбер
	...
	for (int i = 0; i < V; i++)
	{
		...
		for (int j = 0; j < V; j++)
		{
			// редактируем все ненулевые расстояния в исходной матрице
			if (predok[i][j] != 0)
			{
				predok[i][j] = predok[i][j] + distances[i] - distances[j];
			}//  if
		}// for j
	}// for j


	cout << "\n\n\x1b[31m М А Т Р И Ц А   Б Е З   О Т Р И Ц А Т Е Л Ь Н Ы Х   В Е С О В   Р Ё Б Е Р:\x1b[30m\n";
	// отключаем русский язык
	setlocale(0, "C");            // выводим массив в красивых скобках
	cout << setw(2) << setfill(' ') << (char)218 << (char)196 << (char)196 << setw(predok.size() * 2 + 7) << setfill(' ') << (char)196 << (char)196 << (char)191 << endl;          // вывод верхних квадратных скобок
	// Выводим на экран редактированную матрицу смежности без отрицательных весов рёбер
	// идём по всем рёбрам
	for (int i = 0; i < V; i++)
	{
		cout << setw(5) << setfill(' ');
		...
		for (int j = 0; j < V; j++)
		{
			// выводим элемент отредактированной матрицы
			cout << predok[i][j] << " ";
		}// for j
		cout << endl;
	}// for i
	cout << setw(2) << setfill(' ') << (char)192 << (char)196 << (char)196 << setw(predok.size() * 2 + 7) << setfill(' ') << (char)196 << (char)196 << (char)217 << endl;          // вывод верхних квадрат
	// подключаем русский язык
	setlocale(LC_ALL, "Russian");

	if (print)
	{
		// выводим отредактированную матрицу в файл
		fout << "\n\nМ А Т Р И Ц А   Б Е З   О Т Р И Ц А Т Е Л Ь Н Ы Х   В Е С О В   Р Ё Б Е Р:\n";
		for (int i = 0; i < V; i++)
		{
			// идём по всем столбцам
			for (int j = 0; j < V; j++)
	...
	// вектор кратчайших путей для алгоритма Дейкстры от данной вершины до всех остальных
	vector<int> ways(V, 0);
	cout << endl;

	cout << "\n\x1b[33m К Р А Т Ч А Й Ш И Е   Р А С С Т О Я Н И Я:\x1b[30m\n";
	// выводим в файл
	if (print)
	{
		fout << "\nК Р А Т Ч А Й Ш И Е   Р А С С Т О Я Н И Я:\n";
	}
	// алгоритм дейкстры для каждой вершины
	// идём по всем вершинам
	for (int i = 0; i < V; i++)
	{
		// чистим путь от предыдущей вершины до других
		ways.clear();
		// запускаем алгоритм Дейкстры заново для каждой вершины
		Dijkstra(graph, predok, i, ways);

		// выводим вычисленный путь на экран
		// для этого проходим по веутору расстояний
		for (int j = 0; j < ways.size(); j++)
		{
			// если пути между вершинами нет - пропускаем
			if (ways[j] == INF)
				continue;
			// путь из самой в себя тоже пропускаем
			else if (i == j)
				continue;
			//получаем модифицированное расстояние
			ways[j] = ways[j] - distances[i] + distances[j];
			cout << " " << i + 1 << " - " << j + 1 << ": " << ways[j] << endl;
			// выводим в файл
			if (print)
			{
				fout << " " << i + 1 << " - " << j + 1 << ": " << ways[j] << endl;
			}
		}// for j
	}// for i
	return;
...
int main()
{
	...
	// Матрица смежности графа, с которой будем работать, и которую будем изменять
	vector<vector<int>>graph = NewGraph.adjacency_matrix();

	// Вызываем для этой матрицы алгоритм Джонсона
	Djonson(graph, print);
